...
if 'mixer' not in st.session_state:
    ...
    st.session_state['autonomous_manager'] = None
    st.session_state['agent_mode'] = 'Assistant'  # Default mode
...
# --- HELPER: Format telemetry for agents ---
def format_telemetry(data):
    """Creates a concise telemetry string for the assistant"""
...
with st.sidebar:
    ...
    if ASSISTANT_AVAILABLE:
        agent_options.append("Assistant (Consultive)")
    if AUTONOMOUS_AVAILABLE:
        agent_options.append("Autonomous Manager")
    
    if len(agent_options) > 0:
        selected_mode = st.radio(
            "Select Agent Type:",
            agent_options,
            help="Assistant: answers questions | Autonomous: takes actions"
        )
        ...
        # Initialize selected agent if not already done
        if st.session_state['agent_mode'] == 'Assistant':
            if st.session_state['assistant'] is None and ASSISTANT_AVAILABLE:
                try:
            ...
            if st.session_state['autonomous_manager'] is None and AUTONOMOUS_AVAILABLE:
                try:
                    with st.spinner("Initializing Autonomous Manager..."):
                        st.session_state['autonomous_manager'] = AutonomousManager()
                except Exception as e:
                    st.error(f"Failed to initialize Autonomous Manager: {e}")
    else:
        st.warning("No AI agents available")
...
if st.session_state['agent_mode'] == 'Autonomous' and st.session_state['autonomous_manager'] is not None:
    ...
    decision = st.session_state['autonomous_manager'].evaluate_system(data)
...
if st.session_state['agent_mode'] == 'Assistant':
    ...
    if st.session_state['assistant'] is not None:
            ...
            for i, q in enumerate(quick_questions):
                if st.button(q, key=f"quick_{i}", use_container_width=True):
                    ...
                    with st.spinner("Consulting manual..."):
                        response = st.session_state['assistant'].ask(q, telemetry_str)
        ...
        with chat_col:
            ...
            if user_question := st.chat_input("Ask the technical assistant..."):
                ...
                with st.spinner("Consulting manual..."):
                    try:
                        response = st.session_state['assistant'].ask(user_question, telemetry_str)
                        st.session_state['chat_messages'].append({"role": "assistant", "content": response})
                    except Exception as e:
            ...
            with col1:
                with st.form(key='clear_chat_form', clear_on_submit=True):
                    if st.form_submit_button("Clear Chat", use_container_width=True, type="secondary"):
                        st.session_state['chat_messages'] = []
    else:
        st.error("Assistant not initialized. Check sidebar for errors.")
    ...
    if st.session_state['autonomous_manager'] is not None:
        ...
        with col_decision:
            ...
            if st.session_state['last_autonomous_decision']:
                decision = st.session_state['last_autonomous_decision']
                
                # Color code by action
                if decision['action'] == "EMERGENCY_STOP":
                    st.error(f"🚨 **{decision['action']}**")
                elif decision['action'] == "RESET_RELAY":
                    st.warning(f"🔧 **{decision['action']}**")
                elif decision['action'] == "LOG_TICKET":
                    st.info(f"🎫 **{decision['action']}**")
                else:
                    st.success(f"👁️ **{decision['action']}**")
                
                st.caption(f"*{decision['reason']}*")
                
                # Show if LLM was used
                if decision.get('llm_used', False):
                    st.caption("🧠 AI Brain consulted")
                else:
                    st.caption("⚡ Fast heuristics used (tokens saved)")
            else:
                st.info("Waiting for first evaluation...")
        
        with col_actions:
            st.markdown("**Actions Taken:**")
            action_count = len(st.session_state.get('autonomous_actions', []))
            ticket_count = len(st.session_state.get('maintenance_tickets', []))
        ...
        with log_container:
            ...
            messages = st.session_state['chat_messages'][-10:]  # Last 10
            if len(messages) == 0:
                st.info("No autonomous actions yet. System is monitoring...")
            else:
        ...
        if len(st.session_state.get('maintenance_tickets', [])) > 0:
            st.divider()
            st.markdown("**Open Maintenance Tickets:**")
        ...
        st.error("Autonomous Manager not initialized.")
...
with tab4:
    if not df.empty:
        ...
        with col1:
            st.line_chart(df[['timestamp', 'rpm_shaft']].set_index('timestamp'))
        with col2:
            st.line_chart(df[['timestamp', 'fill_level_kg']].set_index('timestamp'))
